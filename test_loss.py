...
class loss(nn.Module):
    ...
    def forward(self,pred,pi,propse_loc,gts):
        ...
        mr = self.compute_mr(l2_norm)
        best_mode = l2_norm.argmin(dim=-1)

        best_traj = pred[torch.arange(pred.size(0)), best_mode]

        propose_loss = l2_norm[torch.arange(pred.size(0)), best_mode].mean()
        ade_loss = torch.norm(best_traj - location, p=2, dim=-1).mean(dim=-1).sum()
        fde_loss = torch.norm(best_traj[:, -1, :] - location[:, -1, :], p=2, dim=-1).sum()


        cls_loss = self.cls_loss(pi,best_mode.to(torch.long))

        return propose_loss, ade_loss, fde_loss, cls_loss,best_traj,mr
    
    def compute_mr(self,l2_norm):
        # l2_norm =l2_norm.reshape(6)
        b,n = l2_norm.shape
        num = 0
        for i in l2_norm:
            for k in i:
